...
//					}
					...
					while(rs.next()) {
						...
						for(int k=1;k<=rsmd.getColumnCount();k++) {
							...
							if(rsmd.getColumnTypeName(k).equals("VARCHAR") || rsmd.getColumnTypeName(k).equals("DATE")||rsmd.getColumnTypeName(k).equals("TIME")) {
								...
								columnStr += Integer.toString(rs.getInt(k)) + "\t";
							}
	...
	class Member extends JFrame {
		LinkedList<Integer> reservation_movies = new LinkedList<>();
		Container c;
		...
		LinkedList<Integer> reservation_numbers = new LinkedList<>();
		LinkedList<Integer> reserved_movie_numbers = new LinkedList<>();
		...
		Member(){
			...
			r_delete.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					System.out.println(reservation_table.getSelectedRow());
					try {
		...
		void searchAllReservation() {
			try {
				...
				stmt = con.createStatement();
				rs = stmt.executeQuery("select m.movie_name, m.movie_date, t.theater_number, t.seat_number, t.ticket_price, t.reservation_number, m.movie_number    \r\n"
						+ "from reservation r\r\n"
				...
				while (rs.next()) {
					...
					reservation_numbers.add(rs.getInt(6));
					reserved_movie_numbers.add(rs.getInt(6));
			...
			public TableCell() {
				...
				jb.addActionListener(e -> {
					...
					try {
						...
						rs = stmt.executeQuery("select seat_number from ticket where ticket_out = FALSE and movietime_number = "+movietime_number+";");
						rs.next();
						seat_number = rs.getInt(1);
						
						rs = stmt.executeQuery("SELECT * FROM ticket WHERE theater_number = "+theater_number+" and seat_number ="+seat_number+";");
						rs.next();
						int reservation_number = seat_number;	// 임의로 이걸로 설정. 이미 있어서 중복되면 어떡하지?
						reservation_movies.add(reservation_number);
						
						// seat_use TRUE로 바꿔줌
						stmt.executeUpdate("UPDATE Seat SET seat_use = TRUE WHERE theater_number = "+theater_number+" and seat_number = "+seat_number+";");
						// 선택된 좌석번호 = 예약번호
						stmt.executeUpdate("INSERT INTO Reservation VALUES("+seat_number+", 'cash', TRUE, 9000, '"+customer_id+"', STR_TO_DATE('2021-12-31','%Y-%m-%d'));");
						...
						// label 만들어서 거기에 이거 출력해주면 됨. settext!!
						//System.out.println(e1.getMessage());
						
					}
					System.out.println(reservation_movies);
				});
